...
void countExponent(int a, int cnt) {
    while (a > 1) { //소인수분해, prime[a] = 소인수,1이라면 더 이상 각각의 소인수에 지수 더해줄 게 없음
        exponent[prime[a]] += cnt; //연산이 곱하기라면 cnt = 1 이여서 지수 증가, 나누기라면 cnt = -1로 지수 감소
        a = a / prime[a];//계속해서 남은 소인수의 지수 더하거나 빼려고 소인수로 나눠줌
    }
}
...
int main() {
    ...
    if (isRationalNumber()) //유리수인 경우 isRationalNumber()은 true반환이어서
        cout << "toothpaste\n";
    else //0 제외 정수인 경우
        cout << "mint chocolate\n";
