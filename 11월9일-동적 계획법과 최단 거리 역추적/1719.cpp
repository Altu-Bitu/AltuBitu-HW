...
int main() {
    ...
    //입력
    cin >> n >> m;
    vector<vector<int>> graph(n + 1, vector<int>(n + 1, INF));//가중치
    vector<vector<int>> table(n + 1, vector<int>(n + 1, 0));//경로
    for (int i = 1; i <= n; i++)
        graph[i][i] = 0;//자기 자신과의 간선 가중치 0으로 초기화

    while (m--) { //무방향 그래프
        cin >> s >> d >> c;
        //간선 정보
        graph[s][d] = graph[d][s] = c;//양방향으로 가중치가 똑같으니까

        //경로 정보
        table[s][d] = d;//중간경로를 일단 도착점으로 해줌
        table[d][s] = s;//위와 같음
    }
