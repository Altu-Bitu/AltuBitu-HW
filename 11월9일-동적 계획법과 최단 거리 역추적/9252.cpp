...
string back(string str1, vector<vector<int>>& path) {
    string result = "";//초기화
    int r = n, c = m;//행 ,열
    while (path[r][c] != -1) {//경로가 없을 때까지
        int d = path[r][c];//경로 유무
        if (d == 2) //좌상향에서 가져온 경우 -> str1[r - 1] == str2[c - 1]
            result += str1[r - 1];
        r += dr[d]; //역추적
        c += dc[d]; //역추적
    }
    reverse(result.begin(), result.end());//순서 역으로 바꿔주기
    return result;//수열 
}

//LCS 길이 구하는 함수
int lcs(string str1, string str2, vector<vector<int>>& path) {
    vector<vector<int>> dp(n + 1, vector<int>(m + 1, 0));//수열 길이
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            if (str1[i - 1] != str2[j - 1]) { //두 문자가 서로 다르면
                ...
                if (dp[i][j] < dp[i][j - 1]) { //왼쪽이 더 크다면
                    dp[i][j] = dp[i][j - 1];//아래쪽 값으로 저장
                    path[i][j] = 1; //경로(방향) 저장
                }
            }
            else if (str1[i - 1] == str2[j - 1]) { //두 문자가 서로 같다면
                dp[i][j] = dp[i - 1][j - 1] + 1;//이 전 수열까지의 길이에서 1 더해주기
                path[i][j] = 2; //경로(방향) 저장
            }
        }
    }
...
int main() {
    ...
    //입력
    cin >> str1 >> str2;
    n = str1.length();// 첫 번째 수열의 길이
    m = str2.length();//두 번째 수열의 길이
