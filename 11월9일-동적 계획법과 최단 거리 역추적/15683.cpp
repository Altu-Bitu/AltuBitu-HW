...
void backtracking(int idx) {
    ...
    int row = idx / m;//row계산
    int col = idx % m;//col계산
    int cur = board[row][col];//해당 위치 board에 저장
    if (cur == 0 || cur == 6 || cur == 7) //cctv가 없는 곳
        return backtracking(idx + 1);//재귀호출

    vector<vector<int>> save = board; //unvisited 처리용 board 상태 저장
    for (int i = 0; i < 4; i++) { //4개의 방향에 대해 cctv 설치
        install(cur, row, col, i);//visited 표시
        backtracking(idx + 1);//재귀호출
        board = save;//unvisited표시
