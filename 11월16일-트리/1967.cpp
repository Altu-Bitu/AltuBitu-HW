...
ci dfs(int node, int parent, vector<vector<ci>>& graph) {
    ...
    for (int i = 0; i < graph[node].size(); i++) {//그래프의 사이즈만큼
        int next_node = graph[node][i].first;//노드 갱신
        if (next_node == parent)//만약 부모노드라면
            continue;//그냥 다음으로

        ci dfs_search = dfs(next_node, node, graph); //자식 노드에 대해 dfs 탐색
        if (graph[node][i].second + dfs_search.first > len) { //기존 거리보다 길다면 갱신
            len = graph[node][i].second + dfs_search.first;//거리 더해주기
            pos = dfs_search.second;//해당 노드를 지름을 구성하는 노드로 바꿔주기
        }
    }
    return { len, pos };//지름을 구성하는 노드랑 노드까지의 거리
}
...
int main() {
    ...
    //입력
    cin >> n;
    vector<vector<ci>> graph(n + 1, vector<ci>(0));//그래프 1번부터
    for (int i = 1; i < n; i++) { //무방향 그래프로 만들기
        cin >> p >> c >> w;//부모 노드 , 자식 노드 , 가중치
        graph[p].emplace_back(c, w);//그래프에 넣어주기
        graph[c].emplace_back(p, w);//그래프에 넣어주기
    }
