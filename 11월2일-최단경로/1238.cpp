...
vector<int> dijkstra(int n, int start, vector<vector<ci>>& graph) {
    ...
    //시작 위치 초기화
    dist[start] = 0;
    pq.push({ 0, start });//우선순위 큐에 넣어주기

    while (!pq.empty()) {//큐가 빌 때까지
        int t = pq.top().first;//이동하는데 걸리는 시간
        int node = pq.top().second;//도착점
        pq.pop();//우선순위 큐에서 제거
        ...
        for (int i = 0; i < graph[node].size(); i++) {
            ...
            if (dist[next_node] > next_t) { //더 짧은 경로로 갈 수 있다면
                dist[next_node] = next_t;//바꿔주기
                pq.push({ next_t, next_node });//소요시간이랑 연결된 정점 우선순위 큐에 넣어주기
            }
        }
    }
    return dist;//소요시간 저장 배열 
}
...
int main() {
    int n, m, x, a, b, c, ans = 0;//마을 수, 도로 수, 모이는 마을 숫자,시작점 ,도착점,소요시간,최장소요시간

    //입력
    cin >> n >> m >> x;
    vector<vector<ci>> graph(n + 1, vector<ci>());//시작점,도착점,소요 시간 저장
    ...
    for (int i = 1; i <= n; i++)
        ans = max(dist_go[i] + dist_back[i], ans);//갔다가 돌아오는 시간이랑 비교
