...
int cntSquare(vector<vector<bool>>& board) {
    int ans = 0;//개수
    for (int i = 0; i < SIZE - 1; i++) {//100*100이니까
        for (int j = 0; j < SIZE - 1; j++) {
            if (board[i][j] && board[i][j + 1] && board[i + 1][j] && board[i + 1][j + 1])
                ans++;//시계방향으로 4점이 있으면 개수 1 더해주기
        }
    }
    return ans;//개수 반환
}
...
ci makeDragonCurve(vector<vector<bool>>& board, vector<int>& direc, ci point) {
    int size = direc.size(); //그 전 세대까지의 방향 정보 개수
    for (int i = size - 1; i >= 0; i--) {
        int new_direc = (direc[i] + 1) % 4; //현재 시작 점의 방향
        point = { point.first + dy[new_direc], point.second + dx[new_direc] }; //현재 끝 점
        board[point.first][point.second] = true;//격자안에 점 있다고 표시
        direc.push_back(new_direc);//현재 시작 점의 방향 삽입
    }
    return point;//현재 끝 점 반환
}
...
int main() {
    ...
    while (n--) {
        ...
        while (g--) { //각 세대마다 드래곤 커브 만들기
            end_point = makeDragonCurve(board, direc, end_point);//마지막 끝점
        }
    }

    //출력
    cout << cntSquare(board) << '\n';//네 꼭짓점이 드래곤 커브의 일부인 것의 개수
    return 0;//종료
}